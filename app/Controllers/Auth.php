...
class Auth extends Controller
{
	...
	public function register(){
        ...
        if($this->request->getMethod() == 'post'){
            ...
            if(!$this->validate($rules)){
                ...
                //     $reason = 'Action';
                //     $message = 'Dear '.$this->request->getVar('firstname', FILTER_SANITIZE_STRING).' '.$this->request->getVar('lastname', FILTER_SANITIZE_STRING).',<br><br> Your Bitsplash Account has been created successfully. <br><br> Kindly click the link below to activate your account.<br>
	...
	public function forgot(){
        $session = session();
		$data['title'] = 'Forgot Password | Bitsplash - Bitcoin Crypto Currency Investment Website';
    ...
    public function reset($uuid=null){
        ...
        if(!empty($uuid)){
            ...
            $user = $user_model->getOne(array('uuid' => $uuid));
            if($user){
                $data['title'] = 'Reset Password | Bitsplash - Bitcoin Crypto Currency Investment Website';
                if($this->request->getMethod() == 'post'){
                    $rules = [
                        'password' => 'required|min_length[5]',
